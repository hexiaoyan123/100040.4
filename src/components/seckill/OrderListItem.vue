      ...
      <div class="movie-item-top" @click="goOrderDetails">
        <div class="movie-item-left">
          <p class="movie-item-left-big">
            ¥ {{info.orderAmount | moneyFormat1}}
            <small>.{{info.orderAmount | moneyFormat2}}</small>
          </p>
          <div class="movie-item-left-stea on">{{info.dataStatus | typeToStr}}</div>
        </div>
        <div class="movie-item-right">
          <h3 class="unoverflow">
            <small>{{info.quantity}}张</small>
          </h3>
          <p class="unoverflow">{{info.phone}}</p>
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  methods: {
    // 查看订单
    goOrderDetails() {
      this.$router.push("/seckill/orderDetails?orderId=" + this.info.id);
    },
