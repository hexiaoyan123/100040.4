<template>
  <div class="container">
    ...
    <div class="ticket-cont">
      <div
        style="margin: 50px auto 5px; text-align: center; font-size: 6.4vw;"
      >{{$init.seckill.name}}</div>
      <div style="margin: 5px auto 30px; text-align: center; font-size: 3.2vw; color: #818181">
        <p>{{order.quantity}}张</p>
      ...
      <div style="margin: 5px 10px 30px; font-size: 3.2vw; color: #818181">
        <p>1.购买活动票成功后，请在电影上映时兑换影票，兑换方式以活动页面说明为准；</p>
        <p>2.请在10分钟内完成支付，超时系统将释放此订单。</p>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    getOrder() {
      let vm = this;
      vm.$http
        .get("/panda-activity-seckill/v1/getOrder/" + vm.orderId)
        .then(function(response) {
          if (response.data.code == "1000") {
            ...
            // 订单支付信息
            let orderPay = {
              business: vm.$init.channelCategory.WAP_SECKILL.code,
              businessName: vm.$init.channelCategory.WAP_SECKILL.msg,
              orderId: vm.order.id,
              overTime: vm.order.overTime,
              orderAmount: vm.order.orderAmount,
              amount: vm.order.orderAmount / vm.order.quantity
            };
            window.localStorage.setItem(
              vm.$init.key.orderPay,
    ...
    confirmPayment() {
      ...
      window.location.href =
        vm.$init.channel.apiUrl +
        "/panda-payment-api/v1/goPayChannel/" +
        vm.$init.login.accessToken +
        "/" +
        vm.$init.channel.id +
        "/" +
        vm.$init.channelCategory.WAP_SECKILL.code +
        "?redirectUrl=" +
        encodeURIComponent(vm.$init.channel.payAuthUrl);
    }
  },
  filters: {
    // 时间格式化
